...
auth_users = [1302933634, 5164955785, 5891177226, 7827086839, 6975428639] # eg: [83528911,836289,9362891] # eg: [83528911,836289,9362891]
...
async def on_start(client: Client, message: Message):
    ...
    await message.reply("<blockquote>Welcome to the best CAMPUS Manga bot!!\n</blockquote>"
                        "\n"
                        ...
                        "<blockquote>𝘰𝘳 𝘤𝘰𝘯𝘵𝘢𝘤𝘵 @aaru_2075</blockquote>")
...
async def favourite_click(client: Client, callback: CallbackQuery):
    ...
    if not subs and fav:
        await db.add(Subscription, Subscription(url=manga.url, user_id=str(callback.from_user.id)))
    if subs and not fav:
        await db.erase(Subscription, subs)
    if subs and fav:
    ...
    if not db_manga:
        await db.add(MangaName, MangaName(url=manga.url, name=manga.name))
    ...
    for url, client in url_client_dictionary.items():
        try:
            ...
            if url not in chapters_dictionary:
                ...
                await db.add(LastChapter, LastChapter(url=url, chapter_url=last_chapter.url))
                ...
                if new_chapters:
                    ...
                    await db.update_last(LastChapter, last_chapter)
